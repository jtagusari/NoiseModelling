...
public class AttenuationOutputSingleThreadTest {
    ...
    private static SceneWithEmission buildScene(String period) {
        ...
        scene.putCnossosParametersPerPeriod(period, attData);
        return scene;
    }
    ...
    void testOnNewCutPlaneProducesFiniteAttenuationForSingleSourceReceiver() {
        ...
        List<ReceiverNoiseLevel> levels = List.copyOf(output.getResultsCache().receiverLevels);
    ...
    void testMergeSourcesFalseProducesOneEntryPerSource() {
        ...
        CalculationIOSettings calcSettings = new CalculationIOSettings.Builder()
                .setMergeSources(false)
                .build();

        AttenuationOutputMultiThread output = new AttenuationOutputMultiThread(scene, new ResultsCache(), calcSettings, new AtomicBoolean(false), new AtomicBoolean(false));
        runPathFinder(scene, output);
    ...
    void testCnossosPathsExportedWhenRaysTableEnabled() {
        ...
        CalculationIOSettings calcSettings = new CalculationIOSettings.Builder()
                ...
                .setExportAttenuationMatrix(false)
                .build();

        AttenuationOutputMultiThread output = new AttenuationOutputMultiThread(scene, new ResultsCache(), calcSettings, new AtomicBoolean(false), new AtomicBoolean(false));
        runPathFinder(scene, output);

        assertFalse(output.getResultsCache().cnossosPaths.isEmpty(),
    ...
    void testWithEmissionNoiseLevelLowerThanSourceEmission() {
        ...
        attData.setHumidity(HUMIDITY);
        attData.setTemperature(TEMPERATURE);
        scene.putCnossosParametersPerPeriod("D", attData);
