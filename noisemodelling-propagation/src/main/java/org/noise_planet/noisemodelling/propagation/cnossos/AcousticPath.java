...
public class AcousticPath extends Path {
    ...
    public AcousticPath(AcousticPathConfiguration configuration) {
     ...
     */
    public AcousticPath buildWithSegmentIndex(int segmentIndex) {
     ...
     */
    private AcousticPath initializePathSource() {
        if (this.hasNoPoints()) {
            ...
            this.addPoint(sourcePoint);
            this.setRaySourceReceiverDirectivity(emissionDirection);
        }
        return this;
    }
     ...
     */
    private AcousticPath addIntermediatePoints() {
    ...
    private void addVerticalEdgeDiffraction(CutPoint currentPoint, int pointIndex, int previousPivotPoint) {
        ...
        this.addPoint(diffractionPoint);
     ...
     */
    private AcousticPath addReceiverPoint() {
        PointPath receiverPoint = new PointPath(
            configuration.getCutPointCoordinates2D().get(endCutPointIndex), 
            endCutPoint.getzGround(), 
            RECV
        );
        this.addPoint(receiverPoint);
        return this;
    }
     ...
     */
    private AcousticPath addMainDiffractionSegment() {
     ...
     */
    private AcousticPath configureDiffractionPointProperties() {
        if (segmentIndex != configuration.getDiffractionPoints().size() - 1) {
            PointPath pt = this.getPointList().get(this.getPointCount() - 1);
            pt.type = DIFH;
