...
note right of step2
  **SQL Operations:**
  • ALTER TABLE ADD SOURCE_TYPE
  • UPDATE SET SOURCE_TYPE='ROAD'
  • INSERT duplicates WHERE BRIDGE_PK IS NOT NULL
  • SET SOURCE_TYPE='BRIDGE' for duplicates
...
1. **Add SOURCE_TYPE Column** — `ALTER TABLE ROADS ADD COLUMN SOURCE_TYPE VARCHAR(20)` adds source type classification field
2. **Initialize All Records** — `UPDATE ROADS SET SOURCE_TYPE='ROAD'` marks all records as road traffic noise sources
3. **Duplicate Bridge Records** — `INSERT INTO ROADS (PK, THE_GEOM, ..., SOURCE_TYPE, BRIDGE_PK) SELECT ..., 'BRIDGE', BRIDGE_PK FROM ROADS WHERE BRIDGE_PK IS NOT NULL` creates duplicate records for roads on bridges
...
PK=1, GEOM=LineString(...), LV_D=100, BRIDGE_PK=NULL, SOURCE_TYPE='ROAD'
PK=2, GEOM=LineString(...), LV_D=200, BRIDGE_PK=5, SOURCE_TYPE='ROAD'

-- After duplication (Step 2 output):
PK=1, GEOM=LineString(...), LV_D=100, BRIDGE_PK=NULL, SOURCE_TYPE='ROAD'
PK=2, GEOM=LineString(...), LV_D=200, BRIDGE_PK=5, SOURCE_TYPE='ROAD'
PK=3, GEOM=LineString(...), LV_D=200, BRIDGE_PK=5, SOURCE_TYPE='BRIDGE'  ← Duplicate
```

**Output:**
`ROADS` table with `SOURCE_TYPE` classification ready for emission calculation (Step 3)

## Step 3: Emission Calculation

The emission generation process transforms road segment data with traffic parameters into sound power level tables. The calculation method is determined by `SOURCE_TYPE` field: CNOSSOS-EU for road traffic noise (`SOURCE_TYPE='ROAD'`) and ASJ for bridge structural noise (`SOURCE_TYPE='BRIDGE'`).

**Calculation Process:**

The emission calculation workflow routes to different calculation engines based on `SOURCE_TYPE`:

### Route 1: Road Traffic Noise (SOURCE_TYPE='ROAD')
...
### Route 2: Bridge Structural Noise (SOURCE_TYPE='BRIDGE')
...
- **SOURCE_TYPE Field:** Preserved from input to distinguish calculation methodology in downstream processing
...
*Table Definition:*
- **27 columns:** `PK` (integer), `THE_GEOM` (geometry), `SOURCE_TYPE` (varchar), `BRIDGE_PK` (integer, nullable), 24 emission levels (double precision)
...
- **SOURCE_TYPE Values:** 'ROAD' for traffic noise, 'BRIDGE' for structural noise

*SQL Workflow:*
1. `CREATE TABLE LW_ROADS (...)` — defines table structure with 27 columns including SOURCE_TYPE and BRIDGE_PK
...
- **SOURCE_TYPE='BRIDGE'**: Creates IMAGINARY_SOURCE_UNDER_BRIDGE during Step 7 sampling for structural noise modeling with negative Z offset

**Output:**
LW_ROADS table with 27 columns containing emission data (24 sound power levels), geometry with Z coordinates, and SOURCE_TYPE classification, ready for geometry loading (Step 5)
...
2. **SourceBridgeProperty Creation:**
   - **No bridges in ProfileBuilder:** Default SourceBridgeProperty (`SOURCE_NOT_RELATED_TO_BRIDGE`)
   - **BRIDGE_PK is NULL or -1:** Default SourceBridgeProperty (`SOURCE_NOT_RELATED_TO_BRIDGE`)
   - **BRIDGE_PK ≥ 0:** Create SourceBridgeProperty based on `SOURCE_TYPE`:
     - `SOURCE_TYPE='ROAD'` → `ACTUAL_SOURCE_ON_BRIDGE(bridgePkOn=BRIDGE_PK, bridgePkAbove=-1)`
     - `SOURCE_TYPE='BRIDGE'` → `IMAGINARY_SOURCE_UNDER_BRIDGE(bridgePkOn=-1, bridgePkAbove=BRIDGE_PK)`
...
1. **Input:**
   ...
   - Source attributes: `sourcePk`, `sourceBridgeProperty`, orientation
...
- Each point retains: position coordinates with **absolute Z**, segment length (`li`), `sourcePk` (for emission data lookup), `sourceBridgeProperty`, orientation
